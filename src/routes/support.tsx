...
const stepsFactory = (t:(s:string)=>string) => [
  { icon: UserPlus, title: t("Create Account"), text: "Sign up in seconds with email or Google." },
  { icon: Search, title: t("Browse Listings"), text: "Filter villas, apartments, and commercial spaces." },
  { icon: CalendarCheck, title: t("Schedule a Tour"), text: "Book a real or virtual tour with a verified broker." },
  { icon: Handshake, title: t("Close the Deal"), text: "Finalize with transparent pricing and full support." },
];

function SupportPage() {
  const t = useT();
  const steps = stepsFactory(t);
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-16">
      <header className="text-center max-w-3xl mx-auto">
        <p className="text-sm font-semibold text-secondary uppercase tracking-wider">{t("Help Center")}</p>
        <h1 className="mt-3 text-4xl md:text-5xl font-bold tracking-tight">{t("How to use our simple interface")}</h1>
        <p className="mt-4 text-muted-foreground">{t("Follow these simple steps to find your dream property.")}</p>
      </header>
        ...
        {steps.map((s, i) => (
          <li key={s.title} className={`relative md:grid md:grid-cols-2 md:gap-12 items-center ${i % 2 ? "md:[&>*:first-child]:order-2" : ""}`}>
            <div className="pl-16 md:pl-0 md:text-right md:pr-12">
              <div className="text-sm font-semibold text-secondary">{t("Step")} {i + 1}</div>
      ...
      <section className="mt-16 grid gap-4 md:grid-cols-3">
        <a href="https://wa.me/20111639205" target="_blank" rel="noopener noreferrer" className="rounded-2xl border bg-card p-5 hover:shadow-elegant transition-shadow">
          <div className="text-xs font-semibold text-secondary uppercase tracking-wider">{t("WhatsApp")}</div>
          <div className="mt-1 font-semibold">+20 111 639 205</div>
          <p className="mt-1 text-sm text-muted-foreground">{t("Tap to chat instantly.")}</p>
        </a>
        <a href="https://t.me/+20111639205" target="_blank" rel="noopener noreferrer" className="rounded-2xl border bg-card p-5 hover:shadow-elegant transition-shadow">
          <div className="text-xs font-semibold text-secondary uppercase tracking-wider">{t("Telegram")}</div>
          <div className="mt-1 font-semibold">+20 111 639 205</div>
          <p className="mt-1 text-sm text-muted-foreground">{t("Send us a Telegram message.")}</p>
        </a>
        <a href="mailto:buildyourhom@gmail.com" className="rounded-2xl border bg-card p-5 hover:shadow-elegant transition-shadow">
          <div className="text-xs font-semibold text-secondary uppercase tracking-wider">{t("Email")}</div>
          <div className="mt-1 font-semibold">buildyourhom@gmail.com</div>
          <p className="mt-1 text-sm text-muted-foreground">{t("Reply within 24 hours.")}</p>
        </a>
      </section>
